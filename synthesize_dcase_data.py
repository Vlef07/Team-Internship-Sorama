...
def _load_audio_mono(file_path: str, sample_rate: int) -> np.ndarray:
    ...
    if sample_width == 1:
        ...
        raise ValueError(f"Unsupported WAV sample width: {sample_width} bytes in {file_path}")
...
def _make_normal_variant(audio: np.ndarray, sample_rate: int, target_samples: int, rng: np.random.Generator) -> np.ndarray:
    ...
    variant = variant + rng.normal(0.0, rng.uniform(0.002, 0.02), size=target_samples).astype(np.float32)
    return _normalize_audio(variant)


def _make_anomaly_variant(audio: np.ndarray, sample_rate: int, target_samples: int, rng: np.random.Generator) -> np.ndarray:
    variant = _make_normal_variant(audio, sample_rate, target_samples, rng).astype(np.float32)
...
def _discover_input_root(preferred: Optional[str] = None) -> Optional[str]:
    ...
    candidates.extend(
        [
            ...
            os.path.join(cwd, "..", "data", "dcase2025t2", "dev_data", "raw"),
            r"C:\Users\20223669\OneDrive - TU Eindhoven\Desktop\Soroma Internship\15097779",
        ]
    )
...
def _synthesize_machine_outputs(
    ...
    for clip in source_clips:
        ...
        for copy_index in range(copies_per_source):
            ...
            train_audio = _make_normal_variant(source_audio, sample_rate, target_samples, rng)
            test_normal_audio = _make_normal_variant(source_audio, sample_rate, target_samples, rng)
            test_anomaly_audio = _make_anomaly_variant(source_audio, sample_rate, target_samples, rng)
...
def main() -> None:
    ...
    input_root = _discover_input_root(args.input_root or args.template_root)
    if not input_root:
        raise FileNotFoundError(
            "No input root with machine/train folders was found. Pass --input-root "
            "pointing at the folder that contains bearing/, fan/, gearbox/, slider/, ToyCar/, "
            "ToyTrain/, and valve/ machine folders."
        )
    ...
    print(f"\nDone. Total generated wav files: {total_files}")
