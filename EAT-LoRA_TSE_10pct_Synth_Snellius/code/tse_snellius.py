"""
this module holds the tse neural network and helper code shared by train and eval
tse takes a noisy waveform and tries to output a cleaner version of the machine sound
step one convert the waveform to a spectrogram with stft
step two run a small u net on the magnitude part to predict a mask between 0 and 1
step three apply the mask and convert back to audio with inverse stft
the network is trained with a negative snr loss against clean target clips
"""
...
from audio_io import load_wav_mono_1d
...
def load_wav_mono(path: str, target_sr: int = TSE_SAMPLE_RATE) -> torch.Tensor:
    ...
    return load_wav_mono_1d(path, target_sr=target_sr)
...
class CrossMachineNoiseBank:
    ...
    def __init__(
        ...
        roots: List[str] = []
        for p in [base_path, *(extra_noise_roots or [])]:
            p = os.path.normpath(str(p))
            if p and p not in roots and os.path.isdir(p):
                roots.append(p)
        if not roots:
            raise FileNotFoundError(f"Geen geldige noise roots (base_path={base_path!r})")
        ...
        for root in roots:
            for machine in sorted(os.listdir(root)):
                if machine == target_machine:
                    continue
                train_dir = os.path.join(root, machine, "train")
        ...
        if not candidates:
            raise RuntimeError(
                f"Geen noise wavs gevonden voor target {target_machine!r} onder roots: {roots}"
            )
...
class TSEMaskNet(nn.Module):
    ...
    def forward(self, wav: torch.Tensor) -> torch.Tensor:
        # step one turn waveform into complex spectrogram
        length = wav.shape[-1]
        ...
        # step two u net predicts a mask on magnitude bins
        e1 = self.enc1(x)
        d1 = self.down1(e1)
        e2 = self.enc2(d1)
        d2 = self.down2(e2)
        ...
        # step three apply mask and convert back to waveform using original phase
